# Agent Instructions for __PROJECT_NAME__
...
The check script runs (in order): codemap regeneration, API client generation, formatting, linting, type checking, tests. All must pass.

## Project context

> **Replace this section.** Describe:
> - What this project does (one paragraph, concrete).
> - The stack (languages, frameworks, key libraries with versions).
...
### Naming
- Files: kebab-case (`user-service.ts`) unless the language convention says otherwise.
- Functions/variables: language-idiomatic (camelCase for JS/TS, snake_case for Python/Rust, etc.).
...
- For frontend changes, run the Playwright recording: `./.agent/scripts/record-feature.sh <feature-name>`. The mp4 lands in `.agent/recordings/`. Review it before declaring done.

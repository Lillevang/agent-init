...
| `--visibility` | How the scaffold is tracked by git. Four planned modes; this build ships `shared`, `local`, and `hidden`. `shared` (default) commits the scaffold normally. `local` appends a fenced, idempotent block to the committed `.gitignore` so the team sees the scaffold is ignored but does not carry the files. `hidden` writes the same block to the never-committed `.git/info/exclude`, leaving zero committed trace (per-repo, local to your clone). Code flavors only; rejected on doc-collab flavors. `global-default` (machine-wide excludes) is an accepted spelling but errors until it lands. |

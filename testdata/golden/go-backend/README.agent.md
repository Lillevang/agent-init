# go-backend — Agentic Development Setup
...
1. Edit `go.mod` and change `module example.com/go-backend` to your actual module path (e.g. `github.com/yourorg/go-backend`).
2. Update the import in `cmd/server/main.go` from `example.com/go-backend/internal/api` to match.
...
```
/workspaces/
├── go-backend/      ← this repo (workspace root, read-write)

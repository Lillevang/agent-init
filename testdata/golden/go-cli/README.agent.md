# go-cli — Agentic Development Setup
...
├── cmd/go-cli/  # binary entry point (path-templated by agent-init)
...
1. Edit `go.mod` and change `module example.com/go-cli` to your actual module path (e.g. `github.com/yourname/go-cli`).
...
Produces `dist/go-cli-linux-amd64`, `-linux-arm64`, `-darwin-arm64`. Add Windows by extending the loop in the `cross-build` recipe.
...
```
/workspaces/
├── go-cli/      ← this repo (workspace root, read-write)
